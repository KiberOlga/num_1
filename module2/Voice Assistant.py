...
r = sr.Recognizer()
voice = pyttsx3.init()
voice.say("Привет! Я твой помогатор!")
voice.runAndWait()


while True:
    with sr.Microphone() as sourse:
        print("Можем говорить")
    ...
    elif speech.find("ютуб") >= 0 or speech.find("youtube"):
        webbrowser.open_new("https://www.youtube.com/")
        voice.say("Я открыла ютуб")
        voice.runAndWait()
    ...
    elif speech.find("пока") >= 0 or speech.find("бай"):
        voice.say("Пока")
        voice.runAndWait()
        break
...
r = sr.Recognizer()
voice = pyttsx3.init()
voice.say('Привет! Меня зовут Элина. Я твой помогатор.')
voice.runAndWait()
...
r = sr.Recognizer()
voice = pyttsx3.init()
voice.say('Привет, меня зовут Элина, я твой голосовой помошник.')
voice.runAndWait()
...
while True:
    ...
    if speech.find('привет') >= 0 or speech.find('добрый день') >= 0:
        greeting = ["Привет!", "Добрый день!", "Hello!", "Hi!", "Good afternoon!"]
        gr = random.choice(greeting)
        voice.say(gr + "у вас очень красивый голос продолжайте")
        voice.runAndWait()
        ...
        voice.say('Я открыла учи ру')
        voice.runAndWait()

    elif speech.find('маил') >= 0 or speech.find('почта') >= 0 or speech.find('почту') >= 0:
        webbrowser.open_new('https://mail.ru/')
        voice.say('Я открыла почту mail')
        voice.runAndWait()

    elif speech.find('решу ЕГЭ') >= 0:
        webbrowser.open_new('https://ege.sdamgia.ru/')
        voice.say('Я открыла сайт решу ЕГЭ')
        voice.runAndWait()

    elif speech.find('молодец') >= 0 or speech.find('умница') >= 0 or speech.find('спасибо') >= 0:
        compliment = ["И вам спасибо, вы очень хороший человек", "Хвалите меня почаще, мне приятно", "Приятно иметь дело с вежливым человеком", "Спасибо за доверие, обращайтесь"]
        ...
        voice.say("у меня сегодня день рождение, мне исполняется 14 лет, так что с вас подарок")
        voice.runAndWait()

    elif speech.find('как тебя зовут') >= 0 or speech.find('как твоё имя') >= 0:
        voice.say("мне кажется у вас проблемы с памятью, мы уже познакомились")
        voice.runAndWait()
        ...
        voice.say('Мне кажется вы задаёте очень много вопросов')
        voice.runAndWait()
        break
    ...
    elif speech.find('имаил') >= 0 or speech.find('email') >= 0:
        try:
