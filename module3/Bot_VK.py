...
#pprint(messages["items"][0])


import vk_api
import time
from course import *
from K_VK_1 import diameter
from K_VK_2 import check_func
    ...
    try:
        ...
        if messages["count"] >= 1:
            ...
            if messages_text.lower() == "планеты":
                vk.method("messages.send", {"peer_id": id, "random_id": messages_id, "message": diameter})
            else:
                vk.method("messages.send", {"peer_id": id, "random_id": messages_id, "message": "что ты от меня хочешь?"})
    except vk_api.exceptions.ApiError as e:
        print(e)
